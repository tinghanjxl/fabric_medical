...
var connection = mysql.createConnection({
	...
	/*router.get('/watchDoctor',function (req,res,next) {

		var pid  = req.query.pid;
		var hid = req.query.hid;
		function backhospitalInfo(pid, callback) {
			connection.query('SELECT *  FROM hospital', function (err, result) {
				if (err)
					callback(err, null);
				else
		...
		backhospitalInfo(pid, function (err, data) {
			if (err) {
				console.log("ERROR : ", err);
			} else {
				...
				console.log(json1);
				res.render('patientAskDoctor.ejs',
					{   hospital:json1,
						pid:pid
				});
